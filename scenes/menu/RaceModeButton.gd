...
func _on_pressed():
	...
	# Use GameOption for mouse control setting
	var game_option = get_node("/root/GameOption")
	car_body.is_mouse_control = game_option.mouse_control_enabled
	
	# Spawn AI opponents
	spawn_ai_opponents(map_instance, car_instance)
	
	map_instance.add_child(car_instance)
	map_instance.gamemode = "Race"
...
func spawn_ai_opponents(map_instance, player_car):
	var ai_car_scene = preload("res://scenes/car/AICar.tscn")
	var num_opponents = 2
	
	for i in range(num_opponents):
		var ai_car = ai_car_scene.instantiate()
		# Offset spawn position
		ai_car.position = player_car.position + Vector2(150 * (i + 1), 50 * (i + 1))
		ai_car.set_player_reference(player_car.get_node("CarBody"))
		map_instance.add_child(ai_car)
